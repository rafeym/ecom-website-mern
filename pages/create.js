...
function CreateProduct() {
  ...
  const handleChange = event => {
    ...
    // To handle media we must use files from event.target to store the actual file not the path
    if (name === 'media') {
      setProduct(prevState => ({ ...prevState, media: files[0] }))
      setMediaPreview(window.URL.createObjectURL(files[0]))
    } else {
      // Use updater pattern to save previous state and append state changes to it
